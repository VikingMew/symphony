defmodule SymphonyElixir.PromptBuilderTest do
  ...
  test "in-repo split package renders correctly" do
    ...
    assert prompt =~ "GitHub push-scope policy (highest priority)"
    assert prompt =~ "first non-empty line of the issue description is exactly `交付路径:宿主 push`"
    assert prompt =~ "git diff --binary origin/main...HEAD"
    assert prompt =~ "需宿主 push"
