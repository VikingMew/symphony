...
| External execution runtime | `docs/execution-runtime-design.md` | Slice 1 provides the separately deployable worker image and worker-side checkout, Codex/hooks, validation, handoff, and local artifact skeleton under one Panel-issued lease. PostgreSQL remains the lifecycle authority; typed Panel metadata/projection and deployment routing remain later slices. |
...
| Deployment and image ownership | `README.md`, `docs/compose.md`, `docs/deployment.md` | Root Compose defaults to building `symphony:local`; its published-image override pulls one selected immutable GHCR multi-architecture image for both migration and application services. GitHub Actions owns amd64/arm64 publication and manifest smoke verification. PostgreSQL is not host-published. Runtime proxy env vars, trusted forwarded headers, health/readiness probes, and reverse-proxy/Kubernetes examples remain supported. Public TLS/domain termination remains external edge infrastructure. |
