...
| GitHub PR-first implementation delivery | `docs/spec-agent-runner.md`, `docs/pull-request-body.md`, `docs/codex-linear-implementation-workflow-design.md`, `docs/user-guide.zh-CN.md` | Codex implements, validates, commits, and pushes the exact Linear branch, then explicitly requests `Ready to Merge`. `AgentRunner` renders the docs-governed initial body from the structured completion result and idempotently ensures the exact open GitHub PR before final Linear writes without overwriting an existing PR. Human review/merge and Linear's merged-PR automation own `Ready to Merge -> Done`; Symphony has no backend merge route. |
