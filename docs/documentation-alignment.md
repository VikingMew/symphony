...
| External execution runtime | `docs/execution-runtime-design.md`, `docs/worker-panel-decoupling-design.md` | The Panel currently snapshots repository/ref/branch, hooks/limits, ordered required gates, and handoff inputs into worker tasks, then validates and transactionally persists bounded runtime/validation/handoff evidence. The separately deployed worker implementation and rollout remain future slices. PostgreSQL remains lifecycle authority. |
