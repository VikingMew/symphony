...
| External execution runtime | `docs/execution-runtime-design.md`, `docs/worker-panel-decoupling-design.md` | Slice 1 provides the separately deployable worker image and worker-side checkout, Codex/hooks, validation, handoff, and local artifact handling under one Panel-issued lease. The Panel snapshots execution inputs and persists bounded runtime, validation, and handoff evidence. PostgreSQL remains lifecycle authority; deployment routing remains a later slice. |
...
| GitHub PR-first implementation delivery | `docs/spec-agent-runner.md`, `docs/codex-linear-implementation-workflow-design.md`, `docs/user-guide.zh-CN.md` | Codex implements, validates, commits, and pushes the exact Linear branch, then explicitly requests `Ready to Merge`. `AgentRunner` idempotently ensures the exact open GitHub PR before final Linear writes. Human review/merge and Linear's merged-PR automation own `Ready to Merge -> Done`; Symphony has no backend merge route. |
...
| Deployment and edge ownership | `README.md`, `docs/compose.md`, `docs/deployment.md` | Root Compose runs PostgreSQL, a one-shot migration release, and a non-root Symphony OTP release with persistent database/log/workspace/credential volumes. PostgreSQL is not host-published. Runtime proxy env vars, trusted forwarded headers, health/readiness probes, and reverse-proxy/Kubernetes examples remain supported. Public TLS/domain termination remains external edge infrastructure. |
