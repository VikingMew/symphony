...
| --- | --- | --- |
| Task queued | Persist `run` and `task` from the current workflow, including issue, repository/ref, prompt/command, project setup commands, hooks, required capabilities, and ordered required gates. | None until a compatible claim succeeds. |
...
| Workspace prepared | Lease remains authoritative and the task snapshot supplies ordered project setup commands with the workspace initialization timeout. | Create a contained checkout/worktree at the requested revision, then run every setup command before hooks or Codex; report the resolved source revision or a typed source/setup failure. |
| Codex / hooks | Payload supplies the structured rendered prompt, app-server settings, hook commands, limits, and handoff policy resolved from the current workflow. | After setup succeeds, run hooks and one Codex app-server JSON-RPC turn in order; report phase, Codex session identifier, bounded redacted detail, duration, and typed outcome. |
