...
2. **有真实 project 时**:`default_project!` 返回 `{:error, :not_found}`(现有行为),
   ...
   Default workflow；没有可用 Default 时回落到第一个 enabled 且已加载 workflow 的真实
   project。未配置仓库地址的 `slug=default` 占位记录即使旧数据中仍为 enabled,也不参与解析。
...
- **runtime 只选择可派发 workflow**:未配置仓库地址的 bootstrap Default 不进入发布快照；
  无上下文 runtime 读取优先配置完整的 Default,否则选择第一个 enabled 且已加载 workflow 的
  project。per-project polling 继续通过 workflow context 解析各自的 Linear slug。
