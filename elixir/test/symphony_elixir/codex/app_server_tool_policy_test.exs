defmodule SymphonyElixir.Codex.AppServerToolPolicyTest do
  ...
  test "app server marks mcp elicitation requests as a hard failure" do
    ...
    try do
      ...
      while IFS= read -r _line; do
        ...
        case "$count" in
          ...
          3)
            ...
            ;;
          4)
            printf '%s\\n' '{"method":"mcpServer/elicitation/request","id":"elicitation-1","params":{"serverName":"linear","toolName":"task_update","prompt":"Approve state change?"}}'
            ;;
          *)
            exit 0
            ;;
      ...
      issue = %Issue{
        ...
        description: "Cannot answer MCP elicitation",
        state: "In Progress",
      ...
      assert get_in(payload, ["params", "serverName"]) == "linear"
      assert get_in(payload, ["params", "toolName"]) == "task_update"
    after
      File.rm_rf(test_root)
    end
  end
    ...
    try do
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 read_tool = Enum.find(tools, &(Map.get(&1, "name") == "linear_task_read"))
                 update_tool = Enum.find(tools, &(Map.get(&1, "name") == "linear_task_update"))

                 ((payload["id"] == 2 and read_tool) && update_tool &&
                    read_tool["description"] =~ "Linear") and update_tool["description"] =~ "Linear" and
                   Map.has_key?(get_in(read_tool, ["inputSchema", "properties"]) || %{}, "activity_limit") and
                   Map.has_key?(get_in(update_tool, ["inputSchema", "properties"]) || %{}, "target_state")
               else
                 false
               end
             end)
