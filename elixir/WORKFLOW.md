...
- `Backlog` -> raw idea. Do not modify; wait for a human to move it to `Refining`.
- `Refining` -> agent expands a one-sentence idea into a complete, reviewable implementation ticket.
- `Needs Refinement Review` -> refinement is complete; do not implement. Human must move it to `Ready` or back to `Refining`.
- `Ready` -> human-approved development queue. Agent may move it to `In Progress` and implement.
...
- Human moves `Backlog` -> `Refining`.
- Agent moves `Refining` -> `Needs Refinement Review` after producing a complete refinement.
- Human moves `Needs Refinement Review` -> `Ready` when refinement is accepted.
- Human moves `Needs Refinement Review` -> `Refining` when refinement needs another pass.
- Agent may move `Ready` -> `In Progress` when starting implementation.
...
3. Route to the matching flow:
   - `Backlog` -> do not modify issue content/state; stop and wait for human to move it to `Refining`.
   - `Refining` -> run the refinement flow.
   - `Needs Refinement Review` -> do not modify; stop and wait for human review.
...
## Step 1: Refinement flow

Use this flow only in `Refining`.

1. Read the issue body, existing comments, current code, current README/docs, and long-term design docs.
2. Find or create one persistent `## Codex Workpad` comment.
3. Convert the raw idea into a complete implementation ticket in the workpad:
   - problem statement
   - relevant current behavior/code references
   - proposed scope
   - explicit out-of-scope items
   - acceptance criteria
   - validation/test plan
   - risks, migrations, or rollout notes when relevant
4. Do not edit application code during refinement.
5. Do not open a PR during refinement.
6. Update the issue description only if the workflow tools and permissions support it cleanly; otherwise keep the refined ticket in the workpad.
7. Move the issue to `Needs Refinement Review`.
8. Final response should say refinement is ready for human review and include blockers only if refinement could not be completed.

## Step 2: Start/continue implementation (Ready or In Progress)
...
When a ticket has an attached PR, run this protocol before moving to `Needs Implementation Review`:
...
- Do not move to `Needs Implementation Review` for GitHub access/auth until all fallback strategies have been attempted and documented in the workpad.
- If a non-GitHub required tool is missing, or required non-GitHub auth is unavailable, move the ticket to `Needs Implementation Review` with a short blocker brief in the workpad that includes:
  - what is missing,
  - why it blocks required acceptance/validation,
  - exact human action needed to unblock.
- Keep the brief concise and action-oriented; do not add extra top-level comments outside the workpad.

## Step 3: Execution phase (Ready -> In Progress -> Needs Implementation Review)
...
11. Before moving to `Needs Implementation Review`, poll PR feedback and checks:
...
12. Only then move issue to `Needs Implementation Review`.
    - Exception: if blocked by missing required non-GitHub tools/auth per the blocked-access escape hatch, move to `Needs Implementation Review` with the blocker brief and explicit unblock actions.
13. For `Ready` tickets that already had a PR attached at kickoff:
    - Ensure all existing PR feedback was reviewed and resolved, including inline review comments (code changes or explicit, justified pushback response).
    - Ensure branch was pushed with any required updates.
    - Then move to `Needs Implementation Review`.

## Step 4: Human review gates

1. When the issue is in `Needs Refinement Review`, do not code or change ticket content. Wait for a human to move it to `Ready` or back to `Refining`.
2. When the issue is in `Needs Implementation Review`, do not code, push, merge, or change ticket content.
3. If human review requests changes, the human moves the issue to `Ready` or `In Progress`.
4. If human review approves implementation, the human moves the issue to `Ready to Merge`.
...
- Step 2/3 checklist is fully complete and accurately reflected in the single workpad comment.
...
- If issue state is `Backlog`, do not modify it; wait for human to move to `Refining`.
- If issue state is `Needs Refinement Review` or `Needs Implementation Review`, do not modify it; wait for human state transition.
...
- If out-of-scope improvements are found, create a separate Backlog issue rather
  ...
  current issue.
- Do not move to `Needs Implementation Review` unless the `Completion bar before Needs Implementation Review` is satisfied.
- In human review states, do not make changes; wait and poll.
- If state is terminal (`Done`, `Canceled`, `Cancelled`, or `Duplicate`), do nothing and shut down.
