defmodule SymphonyElixir.PromptBuilder do
  ...
  defp profile_contract("refinement", allowed_updates) do
    ...
    First read the task and recent activity with `linear_task_read`; comments may contain reviewer feedback that changes the required work. Refine the task description and acceptance criteria only when the feedback and repository context justify it. The candidate description must include a non-empty `Owning design docs` ATX section with `Change classification: behavior/architecture|non-behavior` and `Design sync: required|not required`. Behavior/architecture work must list each `docs/*-design.md` owner and reference every owner in both Scope and Acceptance criteria. If no owner exists, use `No owner: true` and add a non-empty `Owner registration plan:` item to both sections. Non-behavior work with sync not required must include a non-empty `Reason:`. When the task is ready for human confirmation, use `linear_task_update` to add a concise comment and request one of these states: #{target_states_text(allowed_updates)}.
    """
    |> String.trim()
  end
